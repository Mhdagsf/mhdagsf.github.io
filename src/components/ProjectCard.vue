...
<template>
  <article class="brutalist-card flex flex-col group bg-background" :class="project.wide ? 'md:col-span-2 lg:col-span-3' : ''">
    <div class="border-b-4 border-primary flex items-center justify-between" :class="[project.accent, project.wide ? 'px-4 py-3' : 'px-4 py-2.5']">
      <MacDots color-class="bg-white" :size="project.wide ? 'md' : 'sm'" />
      <span class="font-bold text-primary uppercase" :class="project.wide ? 'text-[11px] tracking-[0.15em]' : 'text-[10px] tracking-[0.1em]'">
        {{ project.title }}
      </span>
    </div>

    <div v-if="project.wide" class="flex flex-col lg:flex-row h-full">
      <div class="lg:w-1/2 aspect-video lg:aspect-auto border-b-4 lg:border-b-0 lg:border-r-4 border-primary overflow-hidden relative">
        <img
          :src="project.image"
          :alt="project.title"
      ...
      <div class="lg:w-1/2 p-10 flex flex-col gap-8 justify-center">
        <p class="text-base font-medium leading-relaxed opacity-95">{{ project.description }}</p>
        ...
        <div class="flex gap-6 pt-10 border-t-2 border-dashed border-primary/20 mt-6">
          <a
    ...
    <template v-else>
      <div class="aspect-video bg-surface-variant border-b-4 border-primary overflow-hidden relative">
        <img
          :src="project.image"
          :alt="project.title"
      ...
      <div class="p-8 flex flex-col flex-1 gap-5">
        <p class="text-sm font-medium leading-relaxed opacity-90">{{ project.description }}</p>
